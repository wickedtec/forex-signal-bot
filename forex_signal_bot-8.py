"""
Gold (XAU/USD) Confluence Signal Bot
--------------------------------------
Strategy: Multi-indicator confluence (trend + momentum + volatility + mean-reversion)
          + Smart Money Concepts (market structure / order blocks / FVGs) as primary trigger
Instrument: XAU/USD only (hardcoded -- see SYMBOL below)
Risk model: ATR-adaptive TP/SL, ~3:1 reward:risk

IMPORTANT REALITY CHECK (read before trading real money):
A ~3:1 reward:risk system only needs to win ~25% of trades to break even
(before spread/slippage). That sounds easy, but wide targets can take a
while to hit and often get stopped out first because price chops around
before trending. This script gives you an HONEST backtest of win rate /
expectancy on real data -- trust that number, not the words "insane" or
"high accuracy". Paper trade for at least 100 signals before risking real
capital.

Data source: Twelve Data (https://twelvedata.com) - free tier gives you
800 API credits/day and an 8 credits/minute cap.
...
Usage:
    python forex_signal_bot.py backtest   # backtest last ~5000 bars
    python forex_signal_bot.py live       # check current signal once
    python forex_signal_bot.py watch      # loop + Telegram alerts
    python forex_signal_bot.py sweep      # empirically test filter strictness
...
# Classic trendline break/bounce filter (see compute_trendlines() below) --
# a different technique from both the EMA trend and SMC structure, since
# it's literally connecting swing points with a line rather than using
# moving averages or order-block zones. Off by default: this is new and
# unbacktested against the others, so it shouldn't silently change live
# behavior until you've confirmed it actually helps. Test with
# /backtest?trendline=true or `sweep` before enabling it here.
USE_TRENDLINE_FILTER = False

# This bot trades XAU/USD (gold) only. Hardcoded rather than left as a
# default, so nothing (a stray CLI arg, an old query param) can accidentally
# point it at a different instrument.
SYMBOL = "XAU/USD"

# Session filter: skip low-liquidity hours. Widened to effectively all-day
# (0-24 UTC) rather than the London/NY-only window that makes sense for
# standard forex pairs -- gold has real, tradeable structure during the
# Asian session too (the backtested Aug 5-7 signals all fired 01:45-05:45
# UTC, which a London/NY-only window would have discarded as "off session"
# even though they were valid setups).
...
def fetch_ohlc(pair: str, interval: str = INTERVAL, outputsize: int = OUTPUT_SIZE) -> pd.DataFrame:
    ...
    # XAU/USD is a commodity on Twelve Data, so it includes real volume
    # (unlike pure forex pairs, which typically don't) -- used for VWAP
    # below. Handled defensively in case a plan/interval doesn't return it.
    df["volume"] = pd.to_numeric(df.get("volume"), errors="coerce")
    df = df.sort_values("datetime").reset_index(drop=True)
    return df


PIP_SIZE = 0.01  # gold pip convention (adjust to 0.1 if your broker quotes XAU/USD with 1 decimal)


def pip_size(pair: str = SYMBOL) -> float:
    """Kept as a function (rather than inlining PIP_SIZE everywhere) so
    call sites read the same either way. This bot is XAU/USD-only, so the
    pair argument only exists for compatibility with existing call sites --
...
def vwap(df: pd.DataFrame) -> pd.Series:
    """Daily-anchored VWAP: resets at the start of each UTC calendar day,
    same convention institutions use, rather than an arbitrary rolling
    window. Requires real volume (see fetch_ohlc) -- falls back to NaN if
    volume isn't available for a given bar, rather than silently pretending
    equal weight (that would just be a moving average with extra steps)."""
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Explicit standard deviation / z-score -- Bollinger already uses std
    # internally, but this exposes it directly as "how many std devs is
    # price from its rolling mean right now", which is the number the
    # mean-reversion strategy below actually needs.
    df["std20"] = df["close"].rolling(20).std()
    rolling_mean20 = df["close"].rolling(20).mean()
    df["zscore20"] = (df["close"] - rolling_mean20) / df["std20"].replace(0, np.nan)
...
def compute_trendlines(df: pd.DataFrame) -> pd.DataFrame:
    """Classic technical-analysis trendlines: connect the last two swing lows
    into an ascending support line, and the last two swing highs into a
    descending resistance line, then flag a directional bias per bar when
    price either breaks through one or bounces/rejects off it.

    This is independent of the SMC order-block/FVG logic above -- it's a
    different, more traditional technique (literally drawing a line between
    two points and projecting it forward), layered on the same swing-point
    detection so it doesn't duplicate that work.

    Adds per bar:
      trendline_support     current ascending support line value (or NaN)
      trendline_resistance  current descending resistance line value (or NaN)
      trendline_bias        'bullish' / 'bearish' / None
    """
    df = df.copy()
    n = len(df)
    ...
    support_val = [np.nan] * n
    resistance_val = [np.nan] * n
    trendline_bias = [None] * n

    recent_lows = []   # up to the last 2 (index, price) swing lows
    recent_highs = []  # up to the last 2 (index, price) swing highs
    ...
    for i in range(n):
        ...
        # Ascending support: only valid if the two swing lows are actually
        # rising -- two swing lows that are flat/falling aren't a support
        # trendline, they're just noise or a downtrend.
        if len(recent_lows) == 2:
            (i1, p1), (i2, p2) = recent_lows
            if i2 > i1 and p2 > p1:
                slope = (p2 - p1) / (i2 - i1)
                support_val[i] = p2 + slope * (i - i2)

        # Descending resistance: only valid if the two swing highs are
        # actually falling.
        if len(recent_highs) == 2:
            (i1, p1), (i2, p2) = recent_highs
            if i2 > i1 and p2 < p1:
                slope = (p2 - p1) / (i2 - i1)
                resistance_val[i] = p2 + slope * (i - i2)

        bias = None
        # Breakout above a descending resistance line -- often a trend
        # reversal signal.
        if not np.isnan(resistance_val[i]) and closes[i] > resistance_val[i]:
            bias = "bullish"
        # Breakdown below an ascending support line -- checked after, so a
        # rare simultaneous break both ways doesn't silently favor bullish.
        if not np.isnan(support_val[i]) and closes[i] < support_val[i]:
            bias = "bearish" if bias is None else bias
        # Bounce off support (wicked into it, closed back above) -- trend
        # continuation signal.
        if bias is None and not np.isnan(support_val[i]) and lows[i] <= support_val[i] < closes[i]:
            bias = "bullish"
        # Rejection at resistance (wicked into it, closed back below) --
        # trend continuation signal.
        if bias is None and not np.isnan(resistance_val[i]) and highs[i] >= resistance_val[i] > closes[i]:
            bias = "bearish"

        trendline_bias[i] = bias

    df["trendline_support"] = support_val
    df["trendline_resistance"] = resistance_val
    df["trendline_bias"] = trendline_bias
    return df
...
def generate_signal(row, use_volatility_filter: bool = False, use_smc: bool = None, min_votes: int = None,
                     ...
                     use_trendline_filter: bool = None,
    ...
    if use_trendline_filter is None:
        use_trendline_filter = USE_TRENDLINE_FILTER
    ...
    # --- Trendline break/bounce gate ---
    trendline_buy_ok, trendline_sell_ok = True, True
    if use_trendline_filter:
        trendline_bias = row.get("trendline_bias")
        if trendline_bias is None or (isinstance(trendline_bias, float) and pd.isna(trendline_bias)):
            return None  # no trendline formed yet (needs 2 swing points on each side) -- skip
        trendline_buy_ok = trendline_bias == "bullish"
        trendline_sell_ok = trendline_bias == "bearish"
        if not trendline_buy_ok and not trendline_sell_ok:
            return None
    ...
    if smc_buy_ok and htf_buy_ok and trendline_buy_ok and buy_votes >= min_votes:
        return "BUY"
    if smc_sell_ok and htf_sell_ok and trendline_sell_ok and sell_votes >= min_votes:
        return "SELL"
    return None


def generate_mean_reversion_signal(row, zscore_threshold: float = 2.0, use_session_filter: bool = None,
                                    session_start_hour: int = None, session_end_hour: int = None) -> str:
    """A genuinely different strategy from generate_signal() above -- trades
    AGAINST short-term extremes rather than with the trend. Buys when price
    is stretched abnormally far below its rolling mean, RSI is oversold,
    AND a reversal candle has already formed confirming the bounce is
    underway (never buys purely because price looks "cheap" -- that's how
    you catch a falling knife). Sells the symmetric setup.

    VWAP is used as an ADDITIONAL confirming factor when available, not a
    hard requirement -- Twelve Data's free tier doesn't reliably return
    volume for every instrument/interval, and gating the entire strategy on
    "vwap is not NaN" meant it silently produced zero signals whenever
    volume data happened to be missing, no matter how the zscore threshold
    was tuned. The core signal (zscore + RSI + reversal candle) works with
    price data alone.

    Deliberately does NOT use the SMC/HTF/trend-vote gates from
    generate_signal() -- those exist specifically to require trend
    agreement, which would block most genuine mean-reversion entries by
    design. Combining the two into one signal would make the bot
    contradict itself (one half requiring the trend, the other betting
    against it). Test this as its own strategy -- see
    backtest(..., strategy="mean_reversion") -- rather than assuming it
    improves on the trend-following system.
    """
    if use_session_filter is None:
        use_session_filter = USE_SESSION_FILTER
    if session_start_hour is None:
    ...
    if pd.isna(row.get("zscore20")) or pd.isna(row.get("rsi14")):
    ...
    vwap_available = vwap_val is not None and not pd.isna(vwap_val)
    below_vwap = (not vwap_available) or c < vwap_val   # no VWAP data -> don't block on it
    above_vwap = (not vwap_available) or c > vwap_val

    oversold_stretch = z <= -zscore_threshold and rsi_val < 30 and below_vwap
    overbought_stretch = z >= zscore_threshold and rsi_val > 70 and above_vwap

    bullish_reversal_candle = c > o  # confirms the bounce is already happening
    bearish_reversal_candle = c < o

    if oversold_stretch and bullish_reversal_candle:
        return "BUY"
    if overbought_stretch and bearish_reversal_candle:
        return "SELL"
    return None
...
# ---------------------------------------------------------------------------
def backtest(df: pd.DataFrame, pair: str = SYMBOL, use_atr: bool = None, use_volatility_filter: bool = None,
             use_smc: bool = None, min_votes: int = None, use_htf: bool = None, use_session: bool = None,
             use_trendline: bool = None, htf_already_attached: bool = False, strategy: str = "trend",
             zscore_threshold: float = 2.0) -> dict:
    # Defaults follow the module-level config, but can be overridden per call
    # (used by the /backtest?atr=true&smc=true&min_votes=3&htf=true&session=true&trendline=true web endpoint).
    #
    # strategy="trend" (default) uses generate_signal() -- the existing
    # confluence + SMC + HTF trend-following system.
    # strategy="mean_reversion" uses generate_mean_reversion_signal() instead
    # -- a genuinely different, often opposite-direction strategy. See that
    # function's docstring for why the two aren't combined into one signal.
    # smc/min_votes/htf/trendline params are ignored in mean_reversion mode
    # (they're trend-following concepts that don't apply).
    #
    # htf_already_attached=True skips the internal attach_htf_trend() fetch --
    # use this when the caller has already merged an "htf_trend" column onto
    # `df` itself (e.g. the /sweep endpoint testing many configs on the same
    # data). Without this, running N configs with use_htf=True would make N
    # redundant network calls for identical HTF data, burning through the
    # API's per-minute rate limit for no reason.
    if use_atr is None:
    ...
    if strategy == "trend":
        ...
        if use_htf and not htf_already_attached:
    ...
    while i < n - 1:
        ...
        if strategy == "mean_reversion":
            signal = generate_mean_reversion_signal(row, zscore_threshold=zscore_threshold,
            ...
            signal = generate_signal(row, use_volatility_filter=use_volatility_filter, use_smc=use_smc,
                                      ...
                                      use_trendline_filter=use_trendline,
    ...
    result = summarize(trades, pair, use_atr, use_smc)
    # Diagnostic: makes a missing-volume-data situation visible in the
    # response instead of silently producing zero mean-reversion trades with
    # no clue why. True if ANY bar in this window had real volume.
    result["volume_data_available"] = bool(df["volume"].notna().any()) if "volume" in df.columns else False
    return result
...
# LIVE CHECK
# ---------------------------------------------------------------------------
def check_live(pair: str = SYMBOL, notify: bool = False, use_atr: bool = None, use_volatility_filter: bool = None,
                use_smc: bool = None, min_votes: int = None, use_htf: bool = None, use_session: bool = None,
                use_trendline: bool = None, strategy: str = "trend", zscore_threshold: float = 2.0):
    if use_atr is None:
        use_atr = USE_ATR_TARGETS
    if use_volatility_filter is None:
    ...
    last = df.iloc[-1]
    if strategy == "mean_reversion":
        signal = generate_mean_reversion_signal(last, zscore_threshold=zscore_threshold,
                                                  use_session_filter=use_session,
                                                  session_start_hour=session_start_hour,
                                                  session_end_hour=session_end_hour)
    else:
        signal = generate_signal(last, use_volatility_filter=use_volatility_filter, use_smc=use_smc,
                                  min_votes=min_votes, use_session_filter=use_session, use_htf_filter=use_htf,
                                  use_trendline_filter=use_trendline,
                                  session_start_hour=session_start_hour, session_end_hour=session_end_hour)
    pip = pip_size(pair)
...
def main():
    ...
    mode = sys.argv[1]
    pair = SYMBOL  # this bot is XAU/USD-only; no pair argument needed on the CLI anymore

    if mode == "backtest":
        print(f"\n=== Backtesting {pair} ({INTERVAL}, ATR={USE_ATR_TARGETS}, SMC={USE_SMC_FILTER}, "
              f"min_votes={MIN_CONFLUENCE_VOTES}, session={SESSION_START_HOUR}-{SESSION_END_HOUR}h UTC, "
        ...
        print(f"Watching {pair} every {CHECK_EVERY_SECONDS//60} min. Ctrl+C to stop.")
        while True:
            try:
        ...
        for min_votes in [2, 3, 4]:
            for use_htf in [True, False]:
                for use_smc in [True, False]:
                    for use_trendline in [True, False]:
        ...
        if not results:
            ...
            print(f"  {'votes':<6}{'htf':<6}{'smc':<6}{'trend':<6}{'trades':<8}{'win%':<8}{'breakeven%':<12}{'expectancy(pips)'}")
            for r in results:
                print(f"  {r['min_votes']:<6}{str(r['htf_filter']):<6}{str(r['smc_filter']):<6}"
                      f"{str(r['trendline_filter']):<6}{r['trades']:<8}{r['win_rate_pct']:<8}"
                      f"{r['breakeven_needed_pct']:<12}{r['expectancy_pips']}")
