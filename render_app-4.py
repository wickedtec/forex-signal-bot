...
@app.route("/sweep")
def run_sweep():
    # Visit e.g. /sweep?interval=15min&bars=1500
    #
    ...
    # filter x SMC filter x trendline filter) against the SAME real price
    ...
    # The trendline filter adds no extra API cost at all -- it's computed
    # from the same price data already fetched, no separate data source.
    pair = bot.SYMBOL
    interval = request.args.get("interval", bot.INTERVAL)
    bars = int(request.args.get("bars", 1500))
    try:
