...
def generate_mean_reversion_signal(row, zscore_threshold: float = 2.0, use_session_filter: bool = None,
    ...
    is stretched abnormally far below its rolling mean and VWAP, RSI is
    oversold, AND a reversal candle has already formed confirming the bounce
    is underway (never buys purely because price looks "cheap" -- that's how
    ...
    if pd.isna(row.get("zscore20")) or pd.isna(row.get("rsi14")) or pd.isna(row.get("vwap")):
    ...
    oversold_stretch = z <= -zscore_threshold and rsi_val < 30 and c < row["vwap"]
    overbought_stretch = z >= zscore_threshold and rsi_val > 70 and c > row["vwap"]
